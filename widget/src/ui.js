export const buildBubble = (config) => {
  ...
  // Two icons: chat icon (default) and close icon (when window is open)
  bubble.innerHTML = `
...
export const setBubbleState = (isOpen) => {
  const chatIcon = document.querySelector('.oc-bubble-chat');
  const closeIcon = document.querySelector('.oc-bubble-close');
...
const buildFormFields = (leadFields) => {
  ...
  if (hasServices) {
    const sectionLabel =
      leadFields?.customEnabled && leadFields?.customQuestion
    ...
    if (leadFields.serviceSelectionType === 'dropdown') {
      ...
      const checkboxes = leadFields.services
        .map(
          (s, i) => `
...
const handleFormSubmit = async (leadFields, hasServices, needsContact, onSubmit) => {
  ...
  if (hasServices) {
    if (leadFields.serviceSelectionType === 'dropdown') {
      ...
      const selected = leadFields.services.filter(
        (_, i) => document.getElementById(`oc-service-${i}`)?.checked
      );
...
export const showLeadSuccess = (name) => {
  ...
  aiRow.style.cssText =
    'display:flex;align-items:flex-end;gap:8px;justify-content:flex-end;margin:4px 0;';
  aiRow.innerHTML = `
